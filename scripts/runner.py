...
def run(
    ...
    map_folder_name = f'{n_nodes}_{nodes_skip}_{unconnected_amount}_{unsafe_amount}_{dark_amount}_{run_n}'
    ...
    print(
        f"Total number of nodes: {n_nodes_resulting}. Execution Time: {elapsed_time:.6f} seconds")
...
def runner():
    ...
    for n_nodes in range(
        ...
        for n in range(n_runs_per_n_nodes):
            n_nodes_resulting, elapsed_time = run(
                folder_name, n, n_nodes, nodes_skip, unconnected_amount, unsafe_amount, dark_amount)
